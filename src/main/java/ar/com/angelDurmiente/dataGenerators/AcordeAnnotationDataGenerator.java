...
import ar.com.angelDurmiente.rowProcessors.AcordeAnnotationRowProcessorCommand;
 ...
 */
@Generator(
		objectClass = Acorde.class,
		dependencies = {},
		daoName = "acordeDAO",
		pages = {}
)
public class AcordeAnnotationDataGenerator {

	@ImportFileProcessorRunnerBuilder(fileProcessorDescriptor = FileProcessorDescriptor.class, name = "Importacion de Acordes")
    public ImportFileProcessorRunner prepareImportFileProcessorRunner(FileProcessorDescriptor fileProcessorDescriptor) {
		ImportFileAnnotationProcessorRunner a = new ImportFileAnnotationProcessorRunner(fileProcessorDescriptor, new ExcelFileProcessorCommand(), new AcordeAnnotationRowProcessorCommand());
        return a;
    }
    ...
    public InputStream prepareInputStream() {
        try {
            return FileHelper.findInputStreamInClasspath(ResourceLocation.ACORDES_INIT_DATA_EXCEL_FILE);
        } catch (FileNotFoundException e) {
            throw new DataGeneratorException("File not found [" + ResourceLocation.ACORDES_INIT_DATA_EXCEL_FILE + "].", e);
        }
    }
